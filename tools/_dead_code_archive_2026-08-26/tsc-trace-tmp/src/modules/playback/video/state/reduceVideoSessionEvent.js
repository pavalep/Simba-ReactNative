...
exports.reduceVideoSessionEvent = reduceVideoSessionEvent;
const deriveLoadingState_1 = require("../domain/deriveLoadingState");
function phaseAfterTransport(snapshot) {
    if (snapshot.isSeeking)
        return 'seeking';
    if (snapshot.isBuffering)
        return 'buffering';
    if (snapshot.isPlaying)
        return snapshot.hasFirstFrame ? 'playing' : 'first-frame';
    if (snapshot.hasFirstFrame)
        return 'paused';
    if (snapshot.duration !== null || snapshot.source !== null)
        return 'ready';
    return 'idle';
}
 ...
 * one release longer.
 */
function withLoadingState(snapshot) {
    const loadingState = (0, deriveLoadingState_1.deriveLoadingState)(snapshot);
    return {
        ...snapshot,
        loadingState,
 ...
 */
function reduceVideoSessionEvent(snapshot, event) {
    if (event.type === 'snapshot')
        return withLoadingState(event.snapshot);
    ...
    switch (event.type) {
        ...
        case 'playback-state-changed': {
            if (snapshot.isEnded && !event.isPlaying)
                return snapshot;
