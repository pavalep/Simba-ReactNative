...
- [ ] **T13.01** Eliminate the React `act(...)` warnings. The root cause is TanStack Query's `notifyManager` firing a state update AFTER the test's `act()` block has returned. The `@react-native/jest-preset` already sets `IS_REACT_ACT_ENVIRONMENT = true` (`jest/setup.js:7`), and `@testing-library/react-native`'s `act()` wrapper at `dist/act.js:24-65` saves/restores the flag across its scope. The remaining warnings are NOT silenced by setting the flag again (verified 2026-09-10). The fix is to **mock `notifyManager`** in tests: replace the `notifyManager.setScheduler` shim with a no-op or a direct-call scheduler (`cb => cb()`) so the state update fires synchronously inside the test's `act()` block. Evidence: `npx jest` (no `--forceExit`) reports 10 suites, 119 passed, 1 todo, 0 failures, **0 act warnings**, and the worker exits cleanly. The current microtask shim at `jest.setup.ts:51-57` (V21 P01) is replaced with a sync shim, with a comment explaining the trade-off (sync = warnings about "act not configured" on initial render, microtask = warnings about "not wrapped in act" on async updates — the sync version is the only one that fully silences the warning set).
