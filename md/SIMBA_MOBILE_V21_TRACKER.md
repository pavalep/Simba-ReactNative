...
### W22 F/U #1 — `usePlayWithResume` → `Result` (typed play)

- [x] **T-W22FU-01.01** Change `usePlayWithResume()` return shape from `Promise<boolean>` to `Promise<Result<PlaybackId, StreamError>>` (matching the `usePlaybackFacade().launch.openWithResume` shape from D-023). Bridge `true` → `ok(playbackId)`; bridge `false` → `err(networkError('Player refused resume launch'))`; bridge `throw` → `err(networkError('Player resume launch failed', {cause}))` with both `Error` and non-`Error` coverage. Evidence: `src/infrastructure/player/index.ts:154-187` (commit `05ec5e8`).
- [x] **T-W22FU-01.02** Migrate `useBookmarksScreen.handlePress` to the 4-variant `Result` branching. The P28 partial migration (single generic toast on failure) is replaced with per-variant toasts using the shared `isNetworkError` / `isUnsupportedError` / `isExpiredError` / `isBlockedError` guards. Per-variant strings match `NowPlayingScreen.handleOpenFullPlayer` (T23.07) for consistent UX. Evidence: `src/screens/Bookmarks/hooks/useBookmarksScreen.ts:48-78`.
- [x] **T-W22FU-01.03** Migrate `HistoryScreen.handlePress` to the 4-variant `Result` branching. Pre-W22 the code was `void playWithResume(...)` — a `false` reply was silently swallowed (no toast, no feedback). Now branches on the 4 variants with per-variant toasts (network = "resume will play when online", unsupported = "format not supported", expired = "sign in again", blocked = "not in your region"). Evidence: `src/screens/History/components/HistoryScreen.tsx:88-119`.
- [x] **T-W22FU-01.04** Update `usePlayWithResume.test.tsx` (8 tests) + add 3 new tests for the `Result` branches: `ok(playbackId)` when bridge accepts, `err(networkError)` when bridge returns `false`, `err(networkError)` when bridge throws (Error + non-Error variant). Test count: 73 → 76 player tests. Evidence: `__tests__/infrastructure/player/usePlayWithResume.test.tsx` (commit `05ec5e8`). 23/275/1/0 full suite passing; tsc clean; boundaries 0/0.

### W22 F/U #2 — `PlayerResumeProvider` (bookmark + history fallback)

- [x] **T-W22FU-02.01** Extract the `<SimbaPlayer resumePolicy>` body from App.tsx into a pure helper `resolveResumeMs({bookmarks, history}, id)` at `src/infrastructure/player/resumePolicy.ts` (NEW, ~85 lines). Priority: bookmarks first (explicit user signal; A14 multi-position picks the LATEST — most recently created), history fallback (implicit recent-play signal; 1 entry per fileUri), `undefined` if neither. Defensive `position > 0` guard consistent with the module's `useOpenWithResume.tsx:131`. Evidence: commit `85e5ebe`.
- [x] **T-W22FU-02.02** Re-export the V13/V14 resume-lookup surface from the facade so consumers can opt in without reaching into the module package: `PlayerResumeProvider` (component) + `useOpenWithResume` (auto-lookup hook) + `usePlayItem` (sugar) + types `PlayerResumeLookup` / `PlayerResumeProviderProps` / `ResumePolicy`. Re-export `resolveResumeMs` + `ResumeLookupInput` from the new helper. Evidence: `src/infrastructure/player/index.ts` (commit `85e5ebe`).
- [x] **T-W22FU-02.03** Simplify App.tsx — 14-line policy body collapsed to 6 (read both stores via `getState()`, pass to `resolveResumeMs`). Wrapped in `useCallback(..., [])` so the `<SimbaPlayer>` memo (line 102 of the module's `SimbaPlayer.tsx`) doesn't churn the inner `<PlayerResumeContext>` value. New import: `useRecentHistoryStore`. Evidence: `App.tsx:203-227` (commit `85e5ebe`).
- [x] **T-W22FU-02.04** Add `__tests__/infrastructure/player/resumePolicy.test.ts` (NEW, 10 tests): bookmark match, latest-wins (A14 multi-position), fractional seconds, history fallback, bookmark priority over history, position-0 fallthrough, no-match undefined, empty-store undefined, negative-position defensive guard, both-stores-position-0 undefined. Also fix the pre-existing `usePlayWithResume.test.tsx` tsc error — the test checked `r.error.cause` on the union type, but `cause` only exists on the `NetworkStreamError` variant. Narrowed with `if (r.error.kind === 'network')`. Test count: 75 → 85 player tests. Evidence: commit `85e5ebe`. 24/285/1/0 full suite passing; tsc clean; boundaries 0/0.

### W22 F/U #3 — `renderWithProviders` test helper (1-import-1-wrapper test setup)

- [x] **T-W22FU-03.01** Create `__tests__/helpers/renderWithProviders.tsx` (NEW, ~165 lines) — wraps the standard set (`SafeAreaProvider` + `QueryClientProvider` + `ThemeProvider` + `ToastProvider` + `NavigationContainer` + `Stack.Navigator` + `Stack.Screen`). Screen receives real React Navigation `route` + `navigation` props via `Stack.Screen component={Component}`. Options: `routeName` (typed against `RootStackParamList`) + `initialParams` (typed against `RootStackParamList[RouteName]`) + `extraProviders` opt-in escape hatch. Fresh `QueryClient` per render. Returns `Promise<RenderResult>` (v14 `render` is async). Evidence: commit `34311b8`.
- [x] **T-W22FU-03.02** Add `__tests__/screens/NowPlaying.test.tsx` (NEW, 3 tests) — first end-to-end screen test in the repo. Proves the helper wires correctly: (1) screen root mounts without throwing, (2) `route.params.fileTitle` reaches the screen body via `initialParams`, (3) fallback "Unknown Track" renders when `fileTitle` is omitted. Evidence: commit `34311b8`. 25/288/1/0 full suite passing.
