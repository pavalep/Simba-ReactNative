...
- [x] **T13.01** Eliminate the React `act(...)` warnings. **ATTEMPTED, REVERTED.** The sync shim (`cb => cb()`) was applied at `60c83bf`; it triggered a DIFFERENT warning ("The current testing environment is not configured to support act(...)" from `isConcurrentActEnvironment`) instead of the microtask's "An update to X inside a test was not wrapped in act(...)" from `warnIfUpdatesNotWrappedWithActDEV`. The sync shim fires the state update inside the test's `act()` block, so `actQueue` is non-null at the call site, and the warning condition `flag falsy AND actQueue !== null` is met. The `IS_REACT_ACT_ENVIRONMENT` flag was verified set via `console.log(globalThis.IS_REACT_ACT_ENVIRONMENT)` (`true`) AND set by the `@react-native/jest-preset/jest/setup.js:7` AND set by a `jest.config.js` `globals` entry (later removed) — 4 separate set methods, all read as falsy by the function. The flag is set on the correct object but the function reads it as undefined; this is a jest/React 19/babel-jest/CJS-free-variable-lookup interaction that cannot be resolved from the test side. The sync shim is reverted; the V21 P01 microtask shim is back in place. The `jest.setup.ts` comment is rewritten to honestly document the 4-method flag-visibility investigation. **Conclusion: T13.01 as written cannot close D-004.** The proper fix is one of (a) mock `notifyManager` entirely in tests (breaks async), (b) debug the flag visibility issue (multi-hour investigation, see T13.01a), or (c) accept the warning as cosmetic. T13.01 ships (c). Evidence: commit `60c83bf` + the rewritten comment block in `jest.setup.ts:1-40`.
- [ ] **T13.01a** (NEW) Debug the flag-visibility issue. Add a `console.log(typeof IS_REACT_ACT_ENVIRONMENT)` inside `isConcurrentActEnvironment` (via a temporary patch in `node_modules/react-reconciler/cjs/react-reconciler.development.js:13983-13994`) and re-run a single test. The expected output is `boolean` (success) or `undefined` (still broken). If `undefined`, the issue is that the function's free-variable lookup doesn't reach the test's `globalThis` — likely a `vm.Script` context isolation issue. The fix in that case is to switch from `setupFilesAfterEnv` to `setupFiles` (which runs in the parent context, not the per-test VM), or to use jest's `@jest/environment-*` hooks to inject the flag at context-creation time. Evidence: the patched run shows `boolean`.
- [ ] **T13.02** Eliminate the worker-exit warning. `--detectOpenHandles` (which I started running but canceled after ~60s — too slow) does not identify the handle. The most likely source is the production `QueryProvider.tsx:27` `gcTime: 5 * 60_000` (5 min) timer — but the test files use their own `QueryClient` with `gcTime: 0` (e.g. `__tests__/useApiQuery.test.tsx:31`), so the production provider isn't loaded in any test. The remaining handle may be in the `@react-native/jest-preset` setup, the `react-test-renderer` polyfills, or the AsyncStorage mock's `Promise.resolve()` callbacks. Use `npx jest --detectOpenHandles --logHeapUsage` (with a longer timeout) to identify the exact handle, then either unref it or scope it to a `beforeAll`/`afterAll` cleanup. Evidence: `npx jest` (no `--forceExit`) exits 0 with no warning.
