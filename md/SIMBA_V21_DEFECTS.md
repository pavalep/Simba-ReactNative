...
| **D-004** | `npm test` reports open async handle (worker process fails to exit gracefully) | Caused by D-003 (shim not loaded). Once D-003 lands, the shim is loaded and the warning is gone | OPEN (depends on D-003) |
