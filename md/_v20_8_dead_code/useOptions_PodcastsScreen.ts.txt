// ─── Podcasts Screen — useOptions (browse shell) ──────────────────────
// Owns the runtime shape of the FAB options sheet (multi-select filters
// + sort + view). Co-located with `useSearch` and the screen's data hook
// — all three are siblings in `hooks/`.
