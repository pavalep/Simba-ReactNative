...
export function useVideoPlayerScreen(
  ...
  route: {params?: VideoPlaybackParams},
) {
...
const [autoAdvance, setAutoAdvance] = useState<{uri: string; title: string} | null>(null);
  ...
  const routeSourceRef = useRef<MediaSource | undefined>(routeSource);
  const routeTypeRef = useRef<MediaKind | undefined>(routeType);
  const routeMediaTypeRef = useRef<MediaLane | undefined>(routeMediaType);
  const routeProviderRef = useRef<string | undefined>(routeProvider);
  const routeFolderIdRef = useRef<string | undefined>(routeFolderId);
  ...
  // 46.1: accessibility — high-contrast subtitle override
  ...
  // Apply Settings changes to the active native player without requiring a
  // reload. Unsupported mpv properties are individually guarded by helpers.
  useEffect(() => {
    if (!isReady) return;
    applyPlaybackSettingsToMpv();
    applyAudioSettingsToMpv();
    try {
      MpvPlayer.setProperty('slang', preferredLanguages);
      MpvPlayer.setProperty('sub-auto', settings.isAutoLoadSubtitlesEnabled ? 'fuzzy' : 'no');
      MpvPlayer.setProperty('sub-font-size', sliceSubtitleFontSize);
      MpvPlayer.setProperty('sub-color', highContrastSubtitles ? DEFAULT_SUBTITLE_COLOR : sliceSubtitleTextColor);
      MpvPlayer.setProperty('sub-bg-opacity', highContrastSubtitles ? 0.9 : sliceSubtitleBgOpacity);
    } catch {
      // Native properties are best-effort across Android/iOS player builds.
    }
  }, [
    isReady,
    preferredLanguages,
    settings,
    sliceSubtitleFontSize,
    sliceSubtitleTextColor,
    sliceSubtitleBgOpacity,
    highContrastSubtitles,
  ]);
  ...
  useEffect(() => {
    if (!fileUri && !initialError) {
      const t = setTimeout(() => {
        // 57.1: prefer going back to the originating screen; use Home as the direct cold-start fallback.
    ...
    requestAnimationFrame(() => {
      if (curUri) {
        addRecent({
            ...
            mediaType: routeMediaTypeRef.current ?? getMediaType(curUri),
            type:
              routeTypeRef.current ??
              ((routeMediaTypeRef.current ?? getMediaType(curUri)) === 'audio'
                ? 'audio'
                : 'video'),
            source:
              routeSourceRef.current ??
              (isRemoteUri(curUri) ? 'api' : 'local'),
            provider:
              routeProviderRef.current ??
              (isRemoteUri(curUri) ? sourceFromUri(curUri) : undefined),
            folderId: routeFolderIdRef.current,
          });
      }
      // 57.1: return to the originating screen, or the direct Home root when no back stack exists.
      if (navigation.canGoBack()) {
        navigation.goBack();
      } else {
        navigation.navigateHome?.();
  ...
  const handlePlayPause = useCallback(() => {
    try {
      ...
      // Native state remains authoritative; do not claim a transition in Redux
      // when the bridge rejects the command.
    }
  }, [haptics]);
  ...
  }, [currentIndex, dispatch, loopMode, playlist, queue]);
  ...
  // Explicit bookmark action: one bookmark belongs to one media item. If the
  // item is already bookmarked, the icon updates that bookmark’s position;
  // playback checkpoints update it automatically in the native ticker below.
  const handleToggleBookmark = useCallback(() => {
    ...
    const existing = bookmarksForFile[0];
    if (existing) {
      updateBookmarkPosition({fileUri: uri, position, duration});
      try {
        toast.show('Bookmark position updated', 'success', 1500);
        haptics.light();
      } catch {}
      return;
    }
    ...
    const input = {
      ...
      duration,
      label: '',
      mediaType: routeMediaTypeRef.current ?? getMediaType(uri),
      type:
        routeTypeRef.current ??
        ((routeMediaTypeRef.current ?? getMediaType(uri)) === 'audio' ? 'audio' : 'video'),
      source: routeSourceRef.current ?? (isRemoteUri(uri) ? 'api' : 'local'),
      provider:
        routeProviderRef.current ?? (isRemoteUri(uri) ? sourceFromUri(uri) : undefined),
      folderId: routeFolderIdRef.current,
      thumbnailPath: (() => {
        try {
    ...
    const result = addBookmarkEntry(input);
    if (result.status === 'requires-confirmation') {
      Alert.alert(
        'Bookmark limit reached',
        `Adding “${input.title}” will remove the oldest bookmark “${result.candidate.title}”. Continue?`,
        [
          {text: 'Cancel', style: 'cancel'},
          {
            ...
            style: 'destructive',
            onPress: () => {
              addBookmarkEntry(result.requested, {evictId: result.candidate.id});
            },
          },
        ],
      );
  ...
  const handleAddToPlaylist = useCallback(async () => {
    try {
      ...
      const entry: PlaylistEntry = normalizePlaybackEntry({
        ...
        duration: 0,
        source: isRemoteUri(file.uri) ? 'api' : 'local',
        type: 'video',
        mediaType: 'video',
      });
              ...
              try {
                ...
                if (thumb) {
                  addRecent({
                      ...
                      mediaType: routeMediaTypeRef.current ?? getMediaType(playableUri),
                      type:
                        routeTypeRef.current ??
                        ((routeMediaTypeRef.current ?? getMediaType(playableUri)) === 'audio'
                          ? 'audio'
                          : 'video'),
                      source:
                        routeSourceRef.current ??
                        (isRemoteUri(playableUri) ? 'api' : 'local'),
                      provider:
                        routeProviderRef.current ??
                        (isRemoteUri(playableUri) ? sourceFromUri(playableUri) : undefined),
                      folderId: routeFolderIdRef.current,
                    });
                }
              } catch (e) {
    ...
    const interval = setInterval(() => {
      ...
      if (curPos > 0) {
        ...
        addRecent({
            ...
            duration: MpvPlayer.getDuration?.() ?? 0,
            thumbnailPath: existing?.thumbnailPath,
            mediaType: routeMediaTypeRef.current ?? getMediaType(fileUri ?? ''),
            type:
              routeTypeRef.current ??
              ((routeMediaTypeRef.current ?? getMediaType(fileUri ?? '')) === 'audio'
                ? 'audio'
                : 'video'),
            source:
              routeSourceRef.current ??
              (isRemoteUri(fileUri ?? '') ? 'api' : 'local'),
            provider:
              routeProviderRef.current ??
              (isRemoteUri(fileUri ?? '') ? sourceFromUri(fileUri ?? '') : undefined),
            folderId: routeFolderIdRef.current,
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      ...
      // Automatic position sync never creates bookmarks. It only updates the
      // single entry created by the user’s explicit bookmark action.
      if (bookmarksForFile.length > 0) {
        updateBookmarkPosition({
          fileUri,
  ...
  useEffect(() => {
    ...
    const unsubFile = MpvPlayer.on('onFileLoaded', () => {
      ...
      addRecent({
          ...
          mediaType:
            routeMediaTypeRef.current ?? getMediaType(fileUriRef.current ?? ''),
          type:
            routeTypeRef.current ??
            ((routeMediaTypeRef.current ?? getMediaType(fileUriRef.current ?? '')) ===
            'audio'
          ...
          source:
            routeSourceRef.current ??
            (isRemoteUri(fileUriRef.current ?? '') ? 'api' : 'local'),
          provider:
            routeProviderRef.current ??
            (isRemoteUri(fileUriRef.current ?? '')
              ? sourceFromUri(fileUriRef.current ?? '')
              : undefined),
          folderId: routeFolderIdRef.current,
        });
    ...
    const unsubEnd = MpvPlayer.on('onEndReached', () => {
      ...
      // 31.3: show "Up Next in 5s" before committing the queue/playlist
      // transition. handleNext() resolves the same target again at commit time.
      startAutoAdvance(transition.entry);
    });
